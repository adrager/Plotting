...
void PlotBuilder::plotDataVsBkg(const TString &var, const DataSet *data, const DataSets &bkgs, const DataSets &signals, const HistParams &histParams) const {
 ...
 // hRatioFrame->GetXaxis()->SetNdivisions(12,1,1,"kTrue");  // Change here for anything but NJets
  std::cout<<"Xaxis SetNdivisions set"<<std::endl;
  ...
  hRatioFrame->GetYaxis()->SetTitleOffset(gStyle->GetTitleXOffset());
  hRatioFrame->GetXaxis()->SetTitleSize(0.052);
  hRatioFrame->GetYaxis()->SetTitleSize(0.052);

  // Adapt the top histograms
  hData->GetXaxis()->SetLabelSize(0);
  hData->GetXaxis()->SetTitle("");
 // hData->GetXaxis()->SetNdivisions(12,1,1,"kTrue"); // Change here for anything but NJets
  hData->GetYaxis()->SetTickLength(gStyle->GetTickLength("Y")/0.8);
  hData->GetYaxis()->SetTitleOffset(gStyle->GetTitleXOffset());
  ...
  for(std::vector<TH1*>::iterator it = stack.begin();
    ...
    (*it)->GetYaxis()->SetTitleOffset(gStyle->GetTitleXOffset()+0.05);
    (*it)->GetYaxis()->SetTitleSize(0.052);
  //  (*it)->GetXaxis()->SetNdivisions(12,1,1,"kTrue"); // Change here for anything but NJets
  }
  ...
  hRatioFrame->GetYaxis()->SetTitle("("+dataSetTypeLabel(data->type())+"-"+dataSetTypeLabel(bkgs.front()->type())+")/"+dataSetTypeLabel(bkgs.front()->type())+"");
  ...
  hRatio->Draw("PEsame");
  title->Draw("same");
	leg->SetTextSize(0.03);  //hacked
  leg->Draw("same");
  TPaveText *txt = new TPaveText(1.4,1.41,0.21,0.211,"NDC");
  txt->SetBorderSize(0);
  txt->SetFillColor(0);
  txt->SetTextFont(42);
  txt->SetTextAlign(12);
...
TLegend* PlotBuilder::legend(unsigned int nEntries) const {
  ...
  double margin = 0.02; 
	  double x0 = 0.45;  //hacked
  //double x0 = 0.40; // without yields
  double x1 = 1.-gStyle->GetPadRightMargin()-margin;
  double y1 = 1.-gStyle->GetPadTopMargin()-margin; // hacked remove * 0.8 and the ()
...
TString PlotBuilder::dataSetLegEntry(const TH1* h, const DataSet* ds) const {
  ...
  // Hack to remove number of events in label
//if (false)
//{
